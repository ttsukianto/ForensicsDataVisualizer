...
var RadarChart = {
	...
	d.forEach(function(y, x){
		...
		/*.on('dblclick', function (d){
			...
			g.selectAll(z).transition(200).style("opacity",.5);
		})
		.on('click', function(){
			g.selectAll(z).transition(200).style("opacity", 1);
		});
		.on('click', function (d){
			z = "polygon."+d3.select(this).attr("class");
			sel = d3.select(this);
			...
			if (state) {
				...
				g.selectAll(z).transition(200).style("fill-opacity", cfg.opacityArea);
			}
